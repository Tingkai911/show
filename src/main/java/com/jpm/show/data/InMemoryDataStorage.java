...
public class InMemoryDataStorage {
    ...
    public void addNewTicketsToShow(Show show, List<String> ticketIds) {
        showMap.put(show.getId(), show);
        for(String ticketId : ticketIds) {
            ticketMap.put(ticketId, show.getId());
        }
    }
    ...
    public void removeTicketFromShow(Show show, String ticketId) throws Exception {
        if(!showMap.containsKey(show.getId())) {
            throw new InvalidShowException("Show ID: " + show.getId() + " is not found");
        }
        if(!ticketMap.containsKey(ticketId)) {
            throw new InvalidTicketException("Ticket ID: " + ticketId + " is not found");
        }
