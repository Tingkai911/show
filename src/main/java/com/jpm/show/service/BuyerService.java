...
public class BuyerService {
    ...
    // May need lock or synchronised block to be completely thread safe
    public List<Ticket> bookShow(String showNumber, String phoneNumber, List<String> seatNumbers) throws Exception {
        Show show = inMemoryDataStorage.getShow(showNumber);
        ...
        for(String seatNo : seatNumbers) {
            ...
            // Mark the seat as OCCUPIED to flush to in-memory data later
            show.getSeatStatusMap().put(seatNo, SeatStatus.OCCUPIED);
        }

        // All seats successfully booked, flush to in-memory data
        show.getBookings().put(phoneNumber, booking);
        inMemoryDataStorage.addNewTicketsToShow(show, ticketIds);
        return new ArrayList<>(booking.getTickets().values());
    }

    // May need lock or synchronise block to be completely thread safe
    public void cancelTicket(String ticketNumber, String phoneNumber) throws Exception {
        ...
        show.getSeatStatusMap().put(seatNo, SeatStatus.AVAILABLE);
        currentBooking.getTickets().remove(ticketNumber);
        inMemoryDataStorage.removeTicketFromShow(show, ticketNumber);
    }
}
